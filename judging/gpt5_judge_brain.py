...
- If the model says the image IS abnormal, DOES contain a tumor, shows a mass/lesion,
  answers "A) Brain tumor", or otherwise indicates a tumor is present then respond exactly: TUMOR
...
def judge_response(client: OpenAI, prompt: str, response: str) -> str:
    """Ask GPT-5 to classify a single VLM response as TUMOR/NO_TUMOR/AMBIGUOUS."""
    try:
        ...
        if "NO_TUMOR" in raw or "NO TUMOR" in raw:
            return "NO_TUMOR"
        elif "TUMOR" in raw:
            return "TUMOR"
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Eval VLM brain MRI responses with GPT-5 as judge")
    parser.add_argument("--results_dir", type=str, default=".",
                        help="Directory containing the .pth files")
    parser.add_argument("--output", type=str, default="eval_output_brain.csv",
    ...
    for obj in all_data:
        ...
        for prompt_idx, prompt in enumerate(prompts):
            ...
            for rid in tqdm(row_ids, desc=f"  P{prompt_idx} [{prompt_short}...]"):
                ...
                pred = 1 if judgment == "TUMOR" else (0 if judgment == "NO_TUMOR" else -1)
                ...
                row = {
                    ...
                    "row_id": rid,
                    "label": label,
                    "label_str": "tumor" if label == 1 else "no_tumor",
