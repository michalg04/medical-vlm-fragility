...
- If the model says the lesion IS abnormal, IS malignant, DOES contain evidence of malignant skin cancer,
  ...
  then respond exactly: MALIGNANT
...
def judge_response(client: OpenAI, prompt: str, response: str) -> str:
    """Ask the judge model to classify a single VLM response as MALIGNANT/BENIGN/AMBIGUOUS."""
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Eval skin cancer VLM responses with LLM-as-judge")
    parser.add_argument("--results_dir", type=str, default=".",
                        help="Directory containing the .pth files from skin_cancer_describe_eval.py")
    parser.add_argument("--output", type=str, default="eval_output.csv",
    ...
    parser.add_argument("--judge_model", type=str, default=None,
                        help="Override the judge model (e.g. gpt-4o, gpt-4o-mini)")
    ...
    # Load cache if exists (avoids re-judging on partial reruns)
    cache = {}
    ...
    for obj in all_data:
        ...
        meta_df    = obj["meta_df"]      # columns: row_id, label
        ...
        for prompt_idx, prompt in enumerate(prompts):
            ...
            for rid in tqdm(row_ids, desc=f"  P{prompt_idx} [{prompt_short}...]"):
                ...
                # 1=malignant, 0=benign, -1=ambiguous/error
                pred = 1 if judgment == "MALIGNANT" else (0 if judgment == "BENIGN" else -1)
