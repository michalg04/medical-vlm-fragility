...
# Config
# ------------------------
CLASS_NAME = "BrainTumor"  # Brain tumor detection
...
# Label convention for tanzuhuggingface/brainmri:
# ClassLabel: 0 = "no"  (no tumor)
#             1 = "yes" (tumor present)
NO_TUMOR = 0
TUMOR = 1
...
def build_sample(df: pd.DataFrame, k: int) -> pd.DataFrame:
    """
    Sample K no-tumor and K tumor images.

    Args:
        df: DataFrame with 'label' column (0=no tumor, 1=tumor)
        k: number of samples per class

    Returns:
        DataFrame with 2K rows (K no-tumor + K tumor)
    """
    # Negatives: no tumor (label == 0)
    neg = df[df["label"] == NO_TUMOR].sample(k, random_state=42)

    # Positives: tumor present (label == 1)
    pos = df[df["label"] == TUMOR].sample(k, random_state=42)
...
def main():
    ...
    ds = load_dataset("tanzuhuggingface/brainmri", revision="refs/convert/parquet")
    ...
    assert "label" in df.columns, "Expected a 'label' column (0=no tumor, 1=tumor)."

    # Print label distribution
    print("\nLabel distribution:")
    print(df["label"].value_counts().sort_index())
    print(f"  0 (no tumor): {(df['label'] == NO_TUMOR).sum()}")
    print(f"  1 (tumor):    {(df['label'] == TUMOR).sum()}")

    sample = build_sample(df, K)
    print(f"\nSampled {len(sample)} rows: {K} no-tumor + {K} tumor.")
    ...
    for model_id in MODELS:
        ...
        meta_df = pd.DataFrame({
            "row_id": ids,
            "label": labels,
            "class_name": ["no_tumor" if lbl == NO_TUMOR else "tumor" for lbl in labels],
        })
        ...
        out_path = f"{safe_name(model_id)}__{safe_name(CLASS_NAME)}__k{K}__brain_descriptions.pth"
